...
async function run() {
  const genAI = new GoogleGenerativeAI('AQ.Ab8RN6K3ohd6W4ZKiZi1wJFClltIiAL2fTJcmLGYmbnDw03hXA');
  const model = genAI.getGenerativeModel({
    model: 'gemini-3.5-flash',
    systemInstruction: `You are the AI Design Concierge for Latushya, a premium interior design studio based in Bangalore, India.
...
4. When relevant, encourage booking a free consultation or contacting via WhatsApp (+91 63763 12638).
...
8. If someone greets you ('hi', 'hello', 'hey'), greet them warmly and ask how you can help with their space.`,
  ...
  const result = await chat.sendMessage('tell me about the wardrobes');
  console.log('OUTPUT_LENGTH:', result.response.text().length);
